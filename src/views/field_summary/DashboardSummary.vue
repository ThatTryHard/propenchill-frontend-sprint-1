<template>
  <DashboardLayout>
    <div class="space-y-6 pb-10">
      ...
      <div>
        <h1 class="text-[26px] font-bold text-[#1F2937]">Ringkasan Surat Bidang {{ formattedKategori }}</h1>
        <p class="text-[15px] text-[#6B7280] mt-1">Lihat dan kelola pengajuan surat yang telah dibuat</p>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-4 gap-5">
        <div class="bg-[#E9F3ED] rounded-xl p-5 text-center border border-[#D5E8DD] shadow-sm flex flex-col justify-center items-center h-[120px]">
          <h3 class="text-[#1F2937] font-bold text-[15px] leading-tight mb-2">Total Surat<br>Masuk</h3>
          <p class="text-3xl font-extrabold text-[#1F5C40] mb-1">{{ summaryData.total_masuk }}</p>
          <p class="text-[10px] text-[#6B7280] font-medium">{{ summaryData.masuk_bulan_ini }} surat di bulan ini</p>
        </div>
        
        <div class="bg-[#E9F3ED] rounded-xl p-5 text-center border border-[#D5E8DD] shadow-sm flex flex-col justify-center items-center h-[120px]">
          <h3 class="text-[#1F2937] font-bold text-[15px] leading-tight mb-2">Total Surat<br>Keluar</h3>
          <p class="text-3xl font-extrabold text-[#1F5C40] mb-1">{{ summaryData.total_keluar }}</p>
          <p class="text-[10px] text-[#6B7280] font-medium">{{ summaryData.keluar_bulan_ini }} surat di bulan ini</p>
        </div>

        <div class="bg-[#E9F3ED] rounded-xl p-5 text-center border border-[#D5E8DD] shadow-sm flex flex-col justify-center items-center h-[120px]">
          <h3 class="text-[#1F2937] font-bold text-[15px] leading-tight mb-2">Menunggu<br>Verifikasi</h3>
          <p class="text-3xl font-extrabold text-[#1F5C40] mb-1">{{ summaryData.pending }}</p>
          <p class="text-[10px] text-[#6B7280] font-medium">{{ summaryData.pending }} sedang diproses</p>
        </div>

        <div class="bg-[#E9F3ED] rounded-xl p-5 text-center border border-[#D5E8DD] shadow-sm flex flex-col justify-center items-center h-[120px]">
          <h3 class="text-[#1F2937] font-bold text-[15px] leading-tight mb-2">Surat<br>Ditolak</h3>
          <p class="text-3xl font-extrabold text-[#1F5C40] mb-1">{{ summaryData.ditolak }}</p>
          <p class="text-[10px] text-[#6B7280] font-medium px-2">{{ summaryData.pesanDitolak }}</p>
        </div>
      </div>

      <div class="bg-[#F8FBF9] p-5 rounded-xl shadow-sm border border-[#E5E7EB] flex items-center gap-6">
        
        <div class="flex items-center gap-3 pr-4 h-[56px]">
          <svg xmlns="http://www.w3.org/2000/svg" width="28" height="26" viewBox="0 0 37 34" fill="none" class="text-[#111827]">
          ...
          <span class="text-[17px] font-bold text-[#1F2937]">Filter Data</span>
        </div>
        
        <div class="flex flex-1 items-center gap-5">
          
          <div class="flex items-center gap-3">
            <label class="text-[16px] font-bold text-[#111827] whitespace-nowrap">Tanggal</label>
            <div class="w-[180px] rounded-[12px] p-[2px] shadow-sm bg-[linear-gradient(90.74deg,#3f9760,#d1955f)] transition-all duration-200 hover:shadow-md">
              <div class="bg-[#f8fafc] rounded-[10px] overflow-hidden border-none h-[52px] flex items-center px-[16px]">
                <input 
                  type="date" 
                  v-model="filters.tanggal" 
                  class="w-full text-[16px] font-semibold text-[#111827] bg-transparent outline-none border-none ring-0 cursor-pointer appearance-none flex-row-reverse gap-2 m-0 p-0"
                />
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3">
            <label class="text-[16px] font-bold text-[#111827] whitespace-nowrap">Status</label>
            <VDropdown 
          ...
          <div class="flex items-center gap-3">
            <label class="text-[16px] font-bold text-[#111827] whitespace-nowrap">Klasifikasi</label>
            <VDropdown 
          ...
          <div class="rounded-[12px] p-[2px] shadow-sm bg-[linear-gradient(90.74deg,#3f9760,#d1955f)] transition-all duration-200 hover:shadow-md flex-shrink-0">
            <button @click="resetFilters" class="bg-[#f8fafc] w-[52px] h-[52px] rounded-[10px] flex items-center justify-center outline-none border-none hover:bg-[#e2e3e5] transition-colors cursor-pointer">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M21 12C21 9.61305 20.0518 7.32387 18.364 5.63604C16.6761 3.94821 14.3869 3 12 3C9.48395 3.00947 7.06897 3.99122 5.26 5.74L3 8M8 8H3V3M3 12C3 14.3869 3.94821 16.6761 5.63604 18.364C7.32387 20.0518 9.61305 21 12 21C14.516 20.9905 16.931 20.0088 18.74 18.26L21 16M21 21V16H16" stroke="#111827" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </button>
          </div>

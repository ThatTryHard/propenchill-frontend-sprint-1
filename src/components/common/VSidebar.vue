<template>
  <aside class="flex flex-col h-screen w-[260px] min-w-[260px] bg-gradient-to-b from-[#f0f7f2] to-[#e8f3eb] border-r border-[#d4e8da] py-6 px-4">
    <!-- Logo Section -->
    ...
    <!-- User Profile -->
    <div class="flex items-center gap-3 px-3 py-3 bg-[#f5faf7] rounded-2xl border border-[#d4e8da]">
      <div class="profile-ring flex items-center justify-center w-11 h-11 rounded-full">
        <div class="flex items-center justify-center w-9 h-9 rounded-full bg-[#f0f7f2]">
          <UserRound :size="20" class="text-[#3F9760]" />
        </div>
      </div>
      <div class="flex flex-col leading-tight overflow-hidden">
        <span class="text-[13px] font-semibold text-[#1a202c] truncate">{{ userName }}</span>
        <span class="text-[11px] text-[#718096] truncate">{{ userEmail }}</span>
      </div>
    </div>
  </aside>
</template>
